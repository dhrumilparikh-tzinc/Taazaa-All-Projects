...
def run_test(label, file_path, file_type):
    ...
    with Session(engine) as db:
        ...
        for log in logs:
            print(
                f"  endpoint={log.endpoint} tokens={log.prompt_tokens}+{log.completion_tokens} "
...
def main():
    ...
    items = [
        (label, path, ftype) for label, (path, ftype) in TEST_FILES.items() if Path(path).exists()
    ]
    ...
    for r in results:
        ...
        print(
            f"[{status}] {r['label'].upper()}: keys={r['summary_keys']} "
