...
def main():
    ...
    parser.add_argument(
        "--tolerance", type=float, default=0.05, help="Max allowed regression vs baseline"
    )
    parser.add_argument(
        "--baseline", default="docs/ragas_baseline.json", help="Path to baseline JSON file"
    )
    ...
    if not baseline_path.exists():
        ...
        print(
            "  Run: py scripts/ragas_baseline.py  then copy C:/tmp/ragas_baseline.json → docs/ragas_baseline.json"
        )
        sys.exit(2)
