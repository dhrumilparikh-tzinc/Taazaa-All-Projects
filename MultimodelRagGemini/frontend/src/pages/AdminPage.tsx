...
interface RagasSummaryMetric {
  metric: string;
  current: number | null;
  target: number;
  status: "PASS" | "BELOW_TARGET" | "NO_DATA";
...
function ScoreRing({
  ...
  const ringColor = isBaseline ? "var(--slate)" : "var(--mint-700)";
  const ringStyle: React.CSSProperties = {
    ...
    background:
      score != null
        ? `conic-gradient(${ringColor} ${pct}%, var(--line-2) 0)`
        : "var(--line-2)",
    display: "flex",
    alignItems: "center",
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    if (tab === "ragas" && !ragas) {
      setLoading(true);
      Promise.all([
        api.get("/v1/admin/ragas"),
        api.get("/v1/admin/ragas_summary"),
      ])
        .then(([r, s]) => {
          setRagas(r.data);
      ...
      <div className="page">
        ...
        {tab === "ragas" && !loading && ragas && (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            {/* Metric rings — live DB score, falls back to baseline */}
            <div
              style={{
                display: "flex",
                flexWrap: "wrap",
            ...
            >
              {RAGAS_METRICS.map((key) => {
                const liveScore = ragas.averages?.[key] as number | null | undefined;
                const baselineScore = ragasSummary?.metrics.find((m) => m.metric === key)?.baseline;
                const score = liveScore ?? baselineScore ?? null;
                const isBaseline = liveScore == null && baselineScore != null;
                return (
                  <ScoreRing
                    key={key}
                    score={score}
            ...
            {ragasSummary && ragasSummary.baseline_available && (
              <Panel>
                <PanelTitle>
                  Offline Evaluation Baseline
                  {ragasSummary.run_date && (
                    <span
                      style={{
                        fontSize: "0.75rem",
                        fontWeight: 400,
                  ...
                  <table className="tbl">
                    <thead>
                      <tr>
                        ...
                        <th style={{ textAlign: "right" }}>Live Avg</th>
                        <th style={{ textAlign: "right" }}>Δ vs Baseline</th>
                        <th>Status</th>
                      </tr>
                    </thead>
                    <tbody>
                      {ragasSummary.metrics.map((m) => {
                        const isFaith = m.metric === "faithfulness";
                        const baselineColor =
                          m.baseline != null ? scoreColor(m.baseline, isFaith) : "var(--slate-2)";
                        const liveColor =
                          m.current != null ? scoreColor(m.current, isFaith) : "var(--slate-2)";
                        const deltaColor =
                          m.delta_vs_baseline == null
                            ? "var(--slate-2)"
                            : m.delta_vs_baseline >= 0
                            ? "var(--ok-fg)"
                            : "var(--err-fg)";
                        const statusBg =
                          m.status === "PASS"
                            ? "var(--ok-bg)"
                            : m.status === "BELOW_TARGET"
                            ? "var(--err-bg)"
                            : "var(--line-2)";
                        const statusFg =
                          m.status === "PASS"
                        ...
                        return (
                          <tr key={m.metric}>
                            ...
                            <td
                              className="num"
                              style={{ textAlign: "right", color: baselineColor, fontWeight: 600 }}
                            >
                              {m.baseline != null ? m.baseline.toFixed(4) : "—"}
                            </td>
                            <td
                              className="num"
                              style={{ textAlign: "right", color: "var(--slate-2)" }}
                            ...
                            <td
                              className="num"
                              style={{ textAlign: "right", color: liveColor }}
                            >
                              {m.current != null ? m.current.toFixed(4) : "—"}
                            </td>
                            <td
                              className="num"
                            ...
                            <td>
                              ...
                              >
                                {m.status === "BELOW_TARGET" ? "BELOW TARGET" : m.status}
                              </span>
                            </td>
                          </tr>
                        );
