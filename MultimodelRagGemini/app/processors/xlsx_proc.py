...
headers. Rows beyond _MAX_ROWS (1000) are truncated to keep token budgets
...
row_count) is produced by a single Gemini call on the extracted markdown text.
The full markdown is stored as _chunk_text so the richer content gets embedded
...
_MAX_ROWS = 1000  # hard cap per sheet
...
class XLSXProcessor(BaseProcessor):
    ...
    def summarise(self, text: str, db) -> dict:
        ...
        summary_text = text[:6000] if len(text) > 6000 else text
